...
class ChatRequest(BaseModel):
    ...
    # Sinônimos aceitos para compatibilidade
    ...
    def unify_message(cls, values):
        # Permite usar {"query": ...} ou {"question": ...} além de {"message": ...}
        if not values.get("message"):
            for alt in ("query", "question"):
                if values.get(alt):
                    values["message"] = values.get(alt)
                    break
        return values
