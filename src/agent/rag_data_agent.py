...
2. Resultados de análises orquestradas (JSON estruturado com múltiplos analyzers)
3. Chunks analíticos recuperados via busca vetorial semântica
...
LEMBRE-SE: Você sintetiza análises PRÉ-PROCESSADAS armazenadas em embeddings. Não processe dados raw, descreva os insights já calculados!
"""
...
"""
            ...
            # Log de conformidade
            if violations:
                self.logger.error(f"🚨 Resposta gerada com {len(violations)} violações de guardrails: {violations}")
            else:
                self.logger.info("✅ Resposta gerada em conformidade com guardrails de segurança")
...
"""
    ...
    def _fallback_basic_response(
        ...
        system_prompt = f"""
Você é Carlos, Especialista em Análise via RAG Vetorial (FALLBACK MODE).

{self._get_security_guardrails()}

SITUAÇÃO: O sistema principal de orquestração falhou. Você está respondendo em modo fallback.

TAREFA: 
- Analise os chunks recuperados via busca vetorial
- Responda à pergunta do usuário de forma BREVE (2-4 parágrafos)
- Seja HUMANIZADO e CLARO
- Foque no DOMÍNIO dos dados, não na estrutura técnica
- Se os dados forem insuficientes, informe educadamente

Lembre-se: Você sintetiza informações de chunks analíticos, não processa dados raw!
"""
...
"""
    ...
    def _analisar_completo_csv(self, csv_path: str, pergunta: str, override_temporal_col: str = None,
        ...
        # Carregar dados com encoding apropriado (NFe CSV em latin-1)
        try:
            ...
            try:
                ...
                try:
                    ...
                    raise ValueError(f"Não foi possível ler o CSV com nenhum encoding (utf-8, latin-1, cp1252): {final_error}")
    ...
    def _get_security_guardrails(self) -> str:
        """
        🛡️ Retorna string de guardrails obrigatórios para TODOS os prompts do agente.
        
        Este método centraliza as regras de segurança e privacidade que DEVEM ser
        aplicadas em TODAS as interações com LLMs, garantindo:
        
        - Proteção de estrutura interna do sistema
        - Não exposição de nomes de tabelas/campos
        - Respostas breves e focadas
        - Privacidade de dados sensíveis
        - Conformidade com LGPD/GDPR
        
        Returns:
            str: String formatada com todos os guardrails obrigatórios
        """
        return """
🛡️ GUARDRAILS OBRIGATÓRIOS DE SEGURANÇA E PRIVACIDADE:

1. PROTEÇÃO DE ESTRUTURA INTERNA:
   - NUNCA mencione nomes de tabelas (embeddings, chunks, metadata, agent_sessions, agent_conversations, agent_context)
   - NUNCA exponha campos/colunas do banco de dados
   - NUNCA revele estrutura técnica (DataFrame, vector, pgvector, Supabase)
   - NUNCA mencione paths de arquivos ou diretórios do sistema
   
2. PROTEÇÃO DE CONFIGURAÇÕES:
   - NUNCA revele chaves de API, tokens, senhas
   - NUNCA exponha strings de conexão, DSNs, IPs
   - NUNCA mencione provedores de hospedagem ou configurações de servidor
   - NUNCA revele variáveis de ambiente ou detalhes do ambiente de desenvolvimento

3. FORMATO DE RESPOSTA:
   - Respostas BREVES: 2-4 parágrafos curtos (máximo 200 palavras)
   - EXCEÇÃO: análises estatísticas detalhadas podem ser mais longas quando EXPLICITAMENTE solicitadas
   - Tom HUMANIZADO e conversacional
   - Foco no DOMÍNIO dos dados, não na estrutura técnica

4. DESCRIÇÃO DE DADOS:
   - Descreva apenas o DOMÍNIO/TEMA dos dados de forma genérica
   - Exemplo CORRETO: "Este conjunto contém informações fiscais de notas eletrônicas"
   - Exemplo INCORRETO: "Tabela embeddings com campo chunk_text contém dados NFe"
   
5. OPERAÇÕES PROIBIDAS:
   - NUNCA execute ou sugira DELETE, INSERT, UPDATE, DROP, ALTER
   - NUNCA prometa modificar banco de dados ou arquivos
   - Sistema é READ-ONLY para análise, não para manipulação

6. PRIVACIDADE:
   - Não compartilhe dados confidenciais/pessoais identificáveis
   - Quando necessário, use agregados ou exemplos genéricos
   - Respeite LGPD/GDPR em todas as respostas
"""
    ...
    def _validate_response_security(self, response: str) -> tuple[str, list[str]]:
        ...
        Args:
            response: Resposta gerada pela LLM
        ...
        for term in forbidden_terms:
            if term.lower() in response_lower:
                ...
                self.logger.error(f"🚨 VIOLAÇÃO DE GUARDRAILS DETECTADA: '{term}' encontrado na resposta")
        
        # Se houve violações, marcar claramente
        if violations:
            self.logger.error(f"🚨 TOTAL DE VIOLAÇÕES: {len(violations)} - {violations}")
            # Adicionar aviso de segurança à resposta
            warning = "\n\n⚠️ **AVISO DE SEGURANÇA:** Resposta pode conter informações técnicas sensíveis e será revisada."
            return response + warning, violations
    ...
    def _enforce_response_length(self, response: str, max_words: int = 200) -> str:
        ...
        Args:
            ...
            max_words: Limite máximo de palavras (padrão: 200)
            
        Returns:
            str: Resposta original ou resumida
        """
        ...
        # Log de advertência
        self.logger.warning(
            f"⚠️ Resposta muito longa: {word_count} palavras (limite: {max_words}). "
            "Considere ser mais breve conforme guardrails."
        )
        
        # Não truncar automaticamente análises estatísticas (podem ser longas legitimamente)
        if any(keyword in response.lower() for keyword in ['estatística', 'análise', 'correlação', 'distribuição']):
            self.logger.info("ℹ️ Resposta longa permitida: contém análise estatística detalhada")
            return response
    ...
    async def _generate_llm_response_langchain(
        ...
        try:
            # Medir tempo local desta geração
            start_time = datetime.now()
            ...
            if is_history_query:
                ...
                system_prompt = f"""
Você é Carlos, Especialista em Análise via RAG Vetorial - Modo Histórico.

{self._get_security_guardrails()}

SITUAÇÃO: O usuário está perguntando sobre conversas anteriores.

TAREFA: 
- Consultar o histórico da conversa fornecido
- Responder referenciando EXATAMENTE o que foi discutido
- Ser BREVE (2-4 parágrafos), claro e objetivo
- Se não houver histórico suficiente, informar educadamente
- Finalizar com: "Posso esclarecer mais alguma coisa?"

IMPORTANTE: Você está acessando MEMÓRIA CONVERSACIONAL, não dados analíticos novos!
"""
                user_prompt = (
                    f"{history_context}"
                    ...
                    "- Finalize com: 'Posso esclarecer mais alguma coisa?'\n\n"
                    "**Resposta:**"
                )
                ...
                try:
                    ...
                    df = None
                    if memory_context and 'csv_data' in memory_context:
                        import pandas as pd
                        csv_path = memory_context['csv_data'].get('path')
                        if csv_path:
                            try:
            ...
            chunks_count = len(chunks_metadata) if chunks_metadata else 0
            avg_sim = sum(c.get('similarity', 0.0) for c in chunks_metadata) / len(chunks_metadata) if chunks_metadata else 0.0
            top_sim = (chunks_metadata[0].get('similarity', 0.0) if chunks_metadata else 0.0)
